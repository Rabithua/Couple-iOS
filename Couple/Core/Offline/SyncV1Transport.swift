    ...
    private func prepareAttachments(for operation: PendingOperation) async throws -> PendingOperation {
        ...
        for record in records where requestedIds.contains(record.localId) {
            ...
            guard let width = record.width, let height = record.height else {
                throw SyncTransportError.rejected(String(localized: "待上传照片缺少尺寸信息"))
            }
        ...
        guard serverIds.count == requestedIds.count else {
            throw SyncTransportError.rejected(String(localized: "待上传照片记录不完整"))
        }
...
struct SyncV1Mutation: Encodable, Sendable {
    ...
    init(operation: PendingOperation, deviceId: String) throws {
        guard UUID(uuidString: operation.operationId) != nil,
            ...
            throw SyncTransportError.rejected(String(localized: "Sync v1 身份字段必须是 UUID"))
        }
        guard operation.hlc.wallTimeMilliseconds >= 0,
            ...
            throw SyncTransportError.rejected(String(localized: "Sync v1 HLC 超出服务端支持范围"))
        }
    ...
    private static func validate(_ operation: PendingOperation) throws {
        guard operation.entityType != .attachment else {
            throw SyncTransportError.rejected(String(localized: "attachment 是只读同步实体"))
        }
        ...
        if operation.mutationKind == .delete {
            guard groups == ["lifecycle"], operation.payload.fields.isEmpty else {
                throw SyncTransportError.rejected(String(localized: "delete 只能提交 lifecycle 且不能携带 data"))
            }
            return
        }
        ...
        if operation.mutationKind == .create || operation.mutationKind == .restore {
            guard groups == Set(definitions.keys) else {
                throw SyncTransportError.rejected(String(localized: "create/restore 必须提交实体的全部字段组"))
            }
            ...
            guard requiredFields.isSubset(of: Set(operation.payload.fields.keys)) else {
                throw SyncTransportError.rejected(String(localized: "create/restore 缺少完整 payload"))
            }
            return
        }
        guard !groups.isEmpty else {
            throw SyncTransportError.rejected(String(localized: "update 至少需要一个 changedGroup"))
        }
        for group in groups {
            guard let candidates = definitions[group],
                  !Set(candidates).isDisjoint(with: operation.payload.fields.keys) else {
                throw SyncTransportError.rejected(String(
                    localized: "missingChangedGroupFields",
...
struct SyncV1Entity: Decodable, Sendable {
    ...
    func remoteChange(
        ...
        guard let type = SyncEntityType(rawValue: entityType) else {
            throw APIError.decoding(String(
                localized: "unknownSyncEntityType",
...
struct SyncV1WireHLC: Decodable, Sendable {
    ...
    func timestamp() throws -> HybridLogicalTimestamp {
        guard !wallTimeMs.isEmpty,
            ...
            throw APIError.decoding(String(localized: "HLC wallTimeMs 或 counter 无效"))
        }
